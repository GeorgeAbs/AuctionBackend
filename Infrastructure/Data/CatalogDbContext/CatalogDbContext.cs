...
namespace Infrastructure.Data.CatalogDbContext
{
    public class CatalogDbContext : DbContext, ICatalogDbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder builder)
        {
            ...
            builder.HasDefaultSchema("auction");
            //builder.Entity<ItemTrading>().ToTable("ItemsTrading");

            builder.Entity<ItemTrading>()
                .HasIndex(x => x.MinPrice);

            builder.Entity<ItemTrading>()
                .HasIndex(x => x.MaxPrice);

            builder.Entity<ItemTrading>()
                .HasIndex(x => x.Status);

            builder.Entity<CatalogBoolPropertyName>()
                .HasIndex(x => x.SystemName)
                .IsUnique();

            builder.Entity<CatalogStringPropertyName>()
                .HasIndex(x => x.SystemName)
                .IsUnique();

            builder.Entity<CatalogFloatPropertyName>()
                .HasIndex(x => x.SystemName)
                .IsUnique();

            builder.Entity<CatalogIntPropertyName>()
                .HasIndex(x => x.SystemName)
                .IsUnique();

            builder.Entity<CatalogStringProperty>()
                .HasIndex(x => x.SystemValue)
                .IsUnique();

            builder.Entity<CatalogBoolProperty>()
                .HasIndex(x => x.SystemValue)
                .IsUnique();

            builder.Entity<CatalogIntProperty>()
                .HasIndex(x => x.PropertyValue);

            builder.Entity<CatalogFloatProperty>()
                .HasIndex(x => x.PropertyValue);
        }
    }
    //EntityFrameworkCore\Add-Migration catalog_v_1 -Context CatalogDbContext
}
